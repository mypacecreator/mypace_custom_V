...
<?php //コメントの数によって表示内容をコントロール
	if ($comments) : ?>
<h1><?php comments_number('コメント / トラックバックはありません', 'コメント / トラックバック 1 件', 'コメント / トラックバック % 件' );?></h1>

<!-- ここから個別のコメントのループ　-->
<?php foreach ($comments as $comment) : ?>
<aside id="comment-<?php comment_ID() ?>" <?php comment_class(); ?>>
    ...
    <time pubdate="pubdate"><a href="#comment-<?php comment_ID() ?>" title=""><?php comment_date('Y/m/d H:i') ?> </a></time> <?php edit_comment_link('編集','&nbsp;&nbsp;',''); ?></span>
    <?php comment_text() ?>
</aside>
<?php endforeach; ?>
<!-- /個別のコメントのループ　-->
